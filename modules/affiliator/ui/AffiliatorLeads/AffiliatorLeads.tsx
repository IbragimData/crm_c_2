...
import { useGetAllLeadsByAffiliatorId } from "@/features/affiliator/hooks/useGetAllLeadsByAffiliatorId";
import type { AffiliatorLeadsFilters } from "@/features/affiliator/hooks/useGetAllLeadsByAffiliatorId";
...
import s from "./AffiliatorLeads.module.scss";
...
function countActiveFilters(f: AffiliatorLeadsFilters): number {
  let n = 0;
  const s = f?.status;
...
function getSelectedStatuses(f: AffiliatorLeadsFilters | undefined): LeadStatus[] {
  const s = f?.status;
  if (s == null) return [];
  return Array.isArray(s) ? [...s] : [s];
}

function toggleStatusInFilters(filters: AffiliatorLeadsFilters, st: LeadStatus): AffiliatorLeadsFilters {
...
function isStatusSelected(filters: AffiliatorLeadsFilters | undefined, st: LeadStatus): boolean {
  const s = filters?.status;
  if (s == null) return false;
  return Array.isArray(s) ? s.includes(st) : s === st;
}

interface AffiliatorLeadsProps {
    activeLeads: string[];
...
export function AffiliatorLeads({
  activeLeads,
  setActiveLeads,
  id,
  isOpen,
  ...
  const {
    ...
    setFilters,
    filters,
  } = useGetAllLeadsByAffiliatorId(id || "");
  ...
  const dropdownRef = useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
        setLeadNavigation(leads.map((l) => l.id), "affiliator", id || null);
